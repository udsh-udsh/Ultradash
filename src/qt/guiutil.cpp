...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Ultradash address (e.g. %1)").arg("U7yFR83SQbiezrA72hjcWJtcBip5krte1x"));
...
{
    // return if URI is not valid or is no Ultradash: URI
...
{
    // Convert ultradash:// to ultradash:
    //
    //    Cannot handle this later, because ultradash:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ultradash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Ultradash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Ultradash.lnk
...
{
    return GetAutostartDir() / "ultradash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ultradash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Ultradash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ultradash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ultradash app to startup item list
