...
#if defined(NDEBUG)
#error "Ultradash cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= Params().WalletForkBlock() && nHeight > 400) {
		nSubsidy = 10.70 * COIN;
	}
	else if (nHeight <= 785649950 && nHeight > Params().WalletForkBlock()) {
		nSubsidy = 1 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
{
	RenameThread("ultradash-scriptch");
	scriptcheckqueue.Thread();
}

bool RecalculateUDSHSupply(int nHeightStart)
{
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> udshInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			udshInputs.push_back(stakeIn);
		}
		const bool hasUDSHInputs = !udshInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasUDSHInputs)
					...
					for (const CTxIn& udshIn : udshInputs) {
						if (udshIn.prevout == in.prevout) {
			...
			while (!chainActive.Contains(prev)) {
				...
				for (const CTransaction &t : bl.vtx) {
					for (const CTxIn &in : t.vin) {
						...
						for (const CTxIn &stakeIn : udshInputs) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// Ultradash: We use certain sporks during IBD, so check to see if they are
