...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
			return error("UltradashMiner : generated block is stale");
	}
	...
	if (!ProcessNewBlock(state, NULL, pblock)) {
		return error("UltradashMiner : ProcessNewBlock, block not accepted");
	}
...
{
	LogPrintf("UltradashMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("ultradash-miner");
	...
	while (fGenerateBitcoins || fProofOfStake) {
		...
		LogPrintf("Running UltradashMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
